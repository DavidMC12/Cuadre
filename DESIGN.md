...
colors:
  ...
  money-in: "#059669"
  money-out: "oklch(0.577 0.245 27.325)"
...
Aun heredado, el sistema ya dice algo coherente con el producto. Es sobrio, casi todo en escala de grises, y el color aparece en un solo lugar donde significa algo: la plata. Verde para lo que entra, rojo para lo que sale. Las cifras van en monoespaciada con dígitos tabulares, como en un libro de contabilidad, para que los montos se alineen y se comparen de un vistazo. La densidad es la de una app de uso diario en el celular: textos de 14px, controles de 32px de alto y tarjetas con bordes suaves.
...
- **Gris de Foco** (`oklch(0.708 0 0)`, token `focus-gray`): el anillo de foco del teclado.

### Semantic: money
- **Verde Entrada** (`#059669`, token `money-in`; `#34d399` en oscuro): montos positivos y la barra de ingresos en la tendencia.
- **Rojo Salida** (`oklch(0.577 0.245 27.325)`, token `money-out`; `oklch(0.704 0.191 22.216)` en oscuro): montos negativos y la barra de gastos. **Es el mismo token `destructive`** que usan los errores, "Cerrar sesión" y el aviso de suplantación.
...
### Named Rules
**The Color Means Money Rule.** Fuera de las gráficas, el único color saturado en pantalla es el de un monto: verde si entra, rojo si sale. Todo lo demás es gris.
...
- **Error / Disabled:** borde y anillo en Rojo Salida; los deshabilitados van al 50% de opacidad.
...
Una franja fija de ancho completo en Rojo Salida con texto blanco, arriba de todo: "Mirando la cuenta de …" más un botón "Volver". No se puede cerrar. Es la única superficie grande de color de toda la app.

## Do's and Don'ts

### Do:
- **Do** mostrar cada monto completo, con signo y símbolo, en Geist Mono tabular (componente `Monto`).
- **Do** reservar el verde y el rojo para plata que entra y que sale.
